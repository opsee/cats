...
func mockState(sid StateId, f, n int, t, te time.Time, tw time.Duration) *State {
	return &State{
...
func TestOkToOk(t *testing.T) {
	s := mockState(StateOK, 2, 0, time.Now(), time.Now(), 0)
	r := mockResult(2, 0)
	s.FailingCount = 0
...
func TestFailWaitToFailWait(t *testing.T) {
	s := mockState(StateFailWait, 2, 2, time.Now(), time.Now(), 30*time.Second)
	r := mockResult(2, 2)
	s.FailingCount = 2
...
func TestFailWaitToOk(t *testing.T) {
	s := mockState(StateFailWait, 2, 2, time.Now(), time.Now(), 0)
	r := mockResult(2, 0)
	s.FailingCount = 0
...
func TestFailWaitToFail(t *testing.T) {
	s := mockState(StateFailWait, 2, 2, time.Now(), time.Now().Add(-1*time.Minute), 30*time.Second)
	r := mockResult(2, 2)
	s.FailingCount = 2
...
func TestPassWaitToPassWait(t *testing.T) {
	s := mockState(StatePassWait, 2, 1, time.Now(), time.Now(), 30*time.Second)
	r := mockResult(2, 1)
	s.FailingCount = 1
...
func TestPassWaitToWarn(t *testing.T) {
	s := mockState(StatePassWait, 2, 1, time.Now(), time.Now().Add(-1*time.Minute), 30*time.Second)
	r := mockResult(2, 1)
	s.FailingCount = 1
...
func TestFailToFail(t *testing.T) {
	s := mockState(StateFail, 2, 2, time.Now(), time.Now().Add(-1*time.Minute), 30*time.Second)
	r := mockResult(2, 2)
	s.FailingCount = 2
...
func TestWarnToWarn(t *testing.T) {
	s := mockState(StateWarn, 2, 1, time.Now(), time.Now().Add(-1*time.Minute), 30*time.Second)
	r := mockResult(2, 1)
	s.FailingCount = 1
...
func TestWarnToOk(t *testing.T) {
	s := mockState(StateWarn, 2, 1, time.Now(), time.Now(), 0)
	r := mockResult(2, 0)
	s.FailingCount = 0
